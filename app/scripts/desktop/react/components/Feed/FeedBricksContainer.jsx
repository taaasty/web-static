...
let FeedBricksContainer = React.createClass({
  propTypes: {
  ...
  loadMoreEntries() {
    ...
    let { url, loadLimit } = this.props;

    FeedActionCreators.loadEntries(url, sinceEntryID, loadLimit)
      .then((response) => {
        if (this.isMounted()) {
          this.setState({
...
export default FeedBricksContainer;