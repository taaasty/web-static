...
window.PostEditorLayout = React.createClass
  ...
  getDefaultProps: ->
    ...
    content: '<p>Только бы не сделать его слишком <a href="http://bling.by/map" title="простым">простым</a> и от этого непонятным. Чтобы этого не случилось, давайте поработаем только с текстовыми постами.</p>'
