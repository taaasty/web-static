...
(function (root, factory) {
  ...
  function init(a) {
    ...
    attrs.mask = a.mask || { r: 255, g: 255, b: 0 };
    attrs.debug = a.debug || false;
    attrs.classes = a.classes || {
      dark: 'tlog-headercolor-white',
      light: 'tlog-headercolor-black',
      complex:{
        dark: 'tlog-headercolor-whiteonblack',
        light: 'tlog-headercolor-blackonwhite'
      }
    };
  ...
  function removeClasses(el) {
    ...
    for (var t = 0; t < targets.length; t++) {
      ...
      target.classList.remove(get('classes').complex.light);
      target.classList.remove(get('classes').complex.dark);
    }
  }
  ...
  function calculatePixelBrightness(target) {
    ...
    if (dims.width > 0 && dims.height > 0) {
      ...
      if (pixels != 0) {
        ...
        var receiver = $('body').get(0);

        removeClasses(receiver);

        var cl = mean <= (get('threshold') / 100) ? 'dark' : 'light'

        if (variance > get('minComplexity') / 100) {
          receiver.classList.add(get('classes').complex[cl]);
        }else{
          receiver.classList.add(get('classes')[cl]);
        }
      }
    }
  }
  ...
  function processTargets(checkTarget) {
    ...
    for (var t = 0; t < total; t++) {
      ...
      if (isInside(target, viewport)) {
        ...
        log('Target is not inside viewport');
      }
    }
